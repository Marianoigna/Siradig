with open('extension/popup.js', 'w', encoding='utf-8') as f:
    f.write('''const API_BASE = "https://siradig.onrender.com";
...
document.getElementById("ir_deducciones").addEventListener("click", async () => {
  ...
  if (!tab1 || !tab1.url || !tab1.url.includes("serviciosjava2.afip.gob.ar/radig")) {
    statusEl.textContent = "Primero ingresa a SiRADIG.";
    return;
  }
  statusEl.textContent = "Navegando...";
  await chrome.scripting.executeScript({ target: { tabId: tab1.id }, func: () => { const b = document.querySelector('.btn_empresa'); if (b) b.click(); } });
  setTimeout(async () => {
    await chrome.scripting.executeScript({ target: { tabId: tab1.id }, func: () => { const c = Array.from(document.querySelectorAll('a, span, button')).find(e => e.textContent.trim().toLowerCase() === 'carga de formulario'); if (c) c.click(); } });
  }, 2500);
  setTimeout(async () => {
    await chrome.scripting.executeScript({ target: { tabId: tab1.id }, func: () => { const d = document.querySelector('a[href="#header_deducciones"]') || Array.from(document.querySelectorAll('a[href="#header_deducciones"]')).find(a => a.textContent.toLowerCase().includes('deducciones')); if (d) d.click(); } });
  }, 6000);
...
document.getElementById("auto_cargar").addEventListener("click", async () => {
  ...
  if (!tab2) { statusEl.textContent = "No se encontro pestaña activa."; return; }
  if (tab2.url && (tab2.url.includes("verMenuDeducciones") || tab2.url.includes("verGastosMedicos"))) {
...
''')
print('popup.js limpio con variables separadas')
